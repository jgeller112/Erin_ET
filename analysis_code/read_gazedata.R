...
read_gazedata <- function(gazedata_path, eyes = "both", means_need_both = FALSE,
                          apply_corrections = TRUE, cols_keep=NA) {
  ...
  cols_to_keep <- list(
    Image = "Image1", Time = "RTTime", TobiiTime = "TETTime",
    ...
    DiameterLeft = "DiameterPupilLeftEye", DiameterRight = "DiameterPupilRightEye",
    Degree = "Degree", TrialID = "Condition")
  
  gazedata <- gazedata %>%
    select_(.dots = cols_to_keep) %>%
    mutate_(Origin = ~ "UpperLeft") #DM: need to check this
  ...
  if (eyes == "right") {
    gazedata <- gazedata %>%
      mutate(XMean = ~ XRight, YMean = ~ YRight,
  ...
  gazedata <- gazedata %>%
    mutate_(
      MeanX = ~ compute_monocular_mean(XLeft, XRight),
      MeanY = ~ compute_monocular_mean(YLeft, YRight),
  ...
  cols_in_order <- c(
    "Basename", "Image", "TobiiTime", "Time", "Origin",
     "MeanX", "MeanY","DiameterMean",
    "Degree", "TrialID")
  
  gazedata <- gazedata[cols_in_order]
  
  gazedata <- gazedata %>% rename(Condition="TrialID")
}